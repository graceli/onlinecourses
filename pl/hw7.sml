...
datatype geom_exp = 
	 ...
	 | Var of string
	 | Shift of real * real * geom_exp (* represents a shift expression *)
...
fun eval_prog (e,env) =
	...
	(case List.find (fn (s2,v) => s=s2) env of
      ...
      | Shift(deltaX, deltaY, e) => let val v = eval_prog (e, env)
				    in
					case v of 
					    NoPoints => NoPoints
...
fun preprocess_prog e =
	...
	LineSegment (x1, y1, x2, y2) => if real_close_point (x1, y1) (x2, y2)
					then Point (x1, y1)
					else flip_line_segment (x1, y1, x2, y2)
